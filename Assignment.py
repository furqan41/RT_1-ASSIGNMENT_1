...
def find_golden_token(defult_dist=0.8, rot=50):
	...
	'''
	Used to to detect the closest golden box with the distance of 0.8 to the robot in a cone by default rotation (between -45 and 45 degrees).
	The main purpose here is to have a threshold to stop the robot and avoid walls.
	
	Arguments:
	
		defult_distance = the settable distance of the cone in which the robot can detect golden boxes (by deafault 0.8)
	...
	for token in R.see():
		...
		if token.dist < dist and token.info.marker_type is MARKER_TOKEN_GOLD and -rot < token.rot_y < rot:
	...
	* Fourth step, strictly related to the First, if the robot is close to a golden box, it will call the Rotation() function to turn counter-clockwise with respect to the path.
	...
	while(1):   # Infinite cycle to run the Robot endlessly 
		...
		if find_golden_token() == False:     # No Golden Boxes are detected , First Step.
			...
			#rot = Rotation() # Rotate counter-clockwise
